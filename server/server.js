...
const server = new ApolloServer({
  ...
  context: async ({ req }) => {
    ...
    try {
      ...
      console.error(
        `Unable to authenticate with token ${authToken}, ERROR: ${error}`
      );
    }
...
server
  ...
  .then(({ url }) =>
    console.log(
      ...
      } ${"mode and listening on port:".yellow} ${
        port.yellow.bold.underline
      }. ${"Server started....on url:".yellow} ${url.yellow.bold.underline} `
    )
  );
